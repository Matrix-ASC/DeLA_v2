...
from utils.cutils import generate_spse_matrix, prepare_m, knn_spse, init_coor, la_spse_a4, knn_spse_4n
...
class Stage(nn.Module):
    def __init__(self, args, depth=0):
        ...
        if first:
            ...
            norm = torch.randn(3 * nbr_out_dim)
            self.spse_n = nn.Parameter(norm)
    ...
    def forward(self, x, xyz, prev_knn, indices, pts_list):
        ...
        # rel_cor = rel_cor.view(-1, 3)[::[1307, 523, 131, 31, 17, 7][self.depth]].mul_(self.cor_std)
        ...
        #     torch.save(dist.cpu(), f"dist{self.depth}n.pt")
        #     if self.last:
        #         exit()
        ...
        if self.first:
            ipt = torch.cat([lxyz, F.pad(x, (0, 1))], dim=1).view(1, -1, 12)
            m = prepare_m(self.spse_m).view(self.sp_dim, 12).transpose(0, 1).contiguous()
            m = torch.cat([m, self.spse_c.view(3, self.sp_dim), self.spse_h.view(1, self.sp_dim), F.normalize(self.spse_n.view(3, self.sp_dim), dim=0)], dim=0)
            nbr = knn_spse_4n(ipt, nbr_knn, m, self.training).view(N, -1).sqrt()
        else:
            if self.depth == 1:
                global spse_m
