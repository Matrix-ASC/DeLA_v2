...
__global__ void __launch_bounds__(block_size) knn_spse_backward_kernel(
    float *wgrad,             //      A 12 C
    ...
    // fix bias
    const float4 center = *(float4*)(kxyz + bNn*4);
    ...
    {
        const float4 xyz = *(float4*)(kxyz + b*N*4 + back_idx[idx]*4);
...
void knn_spse_backward(
    ...
    knn_spse_backward_kernel<block_size>
