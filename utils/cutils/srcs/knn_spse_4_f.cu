...
__global__ void __launch_bounds__(block_size) knn_spse_4_forward_kernel(
    float *output,            //      B N C
    uint32_t *back_idx,       //      B N C
    const float *xyz,         //      B N 8
    ...
    if (idx >= BNC) return;
    float proj[16];
    extern __shared__ float nbr_xyz[];  //  block_size / shared_size, k, 8
    ...
    // fix bias
    {
        const float4 center = *(float4*)(xyz + bNn*8);
    ...
    // read nbr xyz
    float *nbr = nbr_xyz + threadIdx.x / shared_size * k * 8;
    for (uint64_t i=idx % shared_size; i<k*8; i+=shared_size)
    {
        const uint64_t nbr_i = nbr_idx[bNn*k + i/8];
        nbr[i] = xyz[b*N*8 + nbr_i*8 + i%8];
    }
    __syncwarp();
    // find min
    float min_dist = 1e8;
    ...
    {
        float dist = calc_distance(*(float4*)(nbr + i*8), proj);
        const float4 nf = *(float4*)(nbr + i*8 + 4);
        const float f1 = nf.x - proj[12], f2 = nf.y - proj[13], f3 = nf.z - proj[14], f4 = nf.w - proj[15];
        dist = dist + f1*f1 + f2*f2 + f3*f3 + f4*f4;
        if (min_dist > dist)
        {
            min_dist = dist;
            min_idx = i;
...
void knn_spse_4_forward(
    ...
    if (C % 32 == 0)
    {
        ...
        knn_spse_4_forward_kernel<shared_size, block_size>
            <<<(B*N*C+block_size-1)/block_size, block_size, block_size/shared_size*k*8*sizeof(float)>>>(
